...
document.querySelectorAll('.add-to-cart').forEach(btn => {
    btn.addEventListener('click', function () {
        ...
        if (confirm("장바구니에 추가 하시겠습니까.")) {
            const finalPriceElement = document.querySelector('#finalPrice'); // finalPrice 요소 선택
            const finalPrice = parseInt(finalPriceElement.getAttribute('value'), 10) || 0; // value 속성에서 숫자 값 가져오기

            const productCart = {
                productId: parseInt(productId, 10),
                cartItemId: 0, // cartItemId는 새로 추가할 때 0으로 설정하거나, 필요 시 서버에서 설정하게 함
                productName: productName,
                originalPrice: originalPrice,
                finalPrice: finalPrice, // long으로 설정된 값
                quantity: quantity,
                file190: file190,
                optionId: parseInt(selectedOptionValue, 10),
                optionName: selectedOptionText,
                point: parseInt(point, 10),
                shippingFee: shippingFee, // int로 설정
                discount: parseInt(discount, 10) // long으로 설정
            };

            console.log("장바구니에 추가할 상품:", productCart);

            // 각 변수도 출력
            console.log("productId:", productCart.productId);
            console.log("cartItemId:", productCart.cartItemId);
            console.log("productName:", productCart.productName);
            console.log("originalPrice:", productCart.originalPrice);
            console.log("finalPrice:", productCart.finalPrice);
            console.log("quantity:", productCart.quantity);
            console.log("file190:", productCart.file190);
            console.log("shippingFee:", productCart.shippingFee);
            console.log("optionId:", productCart.optionId);
            console.log("optionName:", productCart.optionName);
            console.log("point:", productCart.point);
            console.log("discount:", productCart.discount);
